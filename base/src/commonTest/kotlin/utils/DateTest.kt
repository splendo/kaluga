...
class DateTest {
    ...
    @Test
    fun testEquality() {
        val now = KalugaDate.now(locale = Locale.enUsPosix)
        assertEquals(now, now.copy(), "copied Date should be equal")

        val nearEpoch = KalugaDate.epoch(1001)
        assertEquals(KalugaDate.epoch(1001), nearEpoch, "equally created dates should be equal")

        assertEquals(KalugaDate.epoch(1002), nearEpoch + KalugaDate.epoch(1), "Date from addition should be equal")
    }

    @Test
    fun testUTCDate() {
        val utcNow = KalugaDate.nowUtc(locale = Locale.enUsPosix)
        val epochNow = utcNow.millisecondSinceEpoch
        val now = KalugaDate.epoch(epochNow, TimeZone.utc, locale = Locale.enUsPosix)
    ...
    fun testCreateEpochDate() {
        val someDay = KalugaDate.epoch(locale = Locale.enUsPosix).apply {
            year = 2020
            month = 5
            day = 12
            hour = 8
        ...
        val epoch = KalugaDate.epoch(locale = Locale.enUsPosix)
    ...
    @Test
    fun testGet() {
        val someDay = KalugaDate.epoch(574695462750, TimeZone.utc, locale = Locale.enUsPosix)
    ...
    fun testStartOfWeek() {
        ...
        val frenchNow = KalugaDate.now(0, TimeZone.utc, france)
        val usNow = KalugaDate.now(0, TimeZone.utc, us)
    ...
    fun testDaylightSavings() {
        val dayBeforeDLS = KalugaDate.epoch(1616828400000, locale = Locale.createLocale("nl", "NL"), timeZone = TimeZone.get("Europe/Amsterdam")!!)
