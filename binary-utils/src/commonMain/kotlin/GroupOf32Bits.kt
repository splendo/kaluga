...
internal class GroupOf32Bits(
    ...
    companion object {
        operator fun invoke(
            fromInt: Int,
            dataEndianness: Endianness
        ): GroupOf32Bits = GroupOf32Bits(fromInt.toUInt(), dataEndianness)
    }

    constructor(
        fromReader: GATTBinaryReader
    ) : this(fromReader.read32Bits(), fromReader.numberingScheme)
    ...
    override val numberOfBits = UInt.SIZE_BITS
    ...
    override fun copy() = GroupOf32Bits(backingData, octetEndianness)
    override fun reversedOctetOrder() = copy().also(BitGroup<*>::reverseOctets)

    override fun twosComplement(): GroupOf32Bits {
        val complementValue = calculateTwosComplementOf(backingData)
        return GroupOf32Bits(complementValue, octetEndianness)
    }
}
