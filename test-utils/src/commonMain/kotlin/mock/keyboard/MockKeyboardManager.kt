...
class MockKeyboardManager : BaseKeyboardManager {
    ...
    private val _focusHandler: AtomicReference<MockFocusHandler?> = AtomicReference(null)

    private val _isShown = AtomicBoolean(false)
    val isShown get() = _isShown.value

    override fun hide() {
        _isShown.value = false
        _focusHandler.value?.simulateRemoveFocus()
    }

    override fun show(focusHandler: FocusHandler) {
        _isShown.value = true
        _focusHandler.value = focusHandler as? MockFocusHandler
        _focusHandler.value?.simulateGiveFocus()
    }
}
