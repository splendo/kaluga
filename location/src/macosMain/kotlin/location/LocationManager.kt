...
/**
 * macOS implementation of [BaseLocationManager].
 *
 * The iOS implementation wires up its `CLLocationManager` delegate through a Swift-interop wrapper
 * (`KalugaLocationWrapper`); on macOS we use a plain Kotlin/Native [NSObject] subclass that
 * conforms to [CLLocationManagerDelegateProtocol] directly. The `background` flag of
 * [LocationPermission] is ignored — macOS apps don't have an iOS-style foreground/background
 * distinction for location updates.
 */
actual class DefaultLocationManager(settings: Settings, coroutineScope: CoroutineScope) : BaseLocationManager(settings, coroutineScope) {
    ...
    private class Delegate(private val onLocationsChanged: MutableSharedFlow<Location.KnownLocation>) :
        ...
        override fun locationManager(manager: CLLocationManager, didUpdateLocations: List<*>) {
            ...
            locations.forEach { onLocationsChanged.tryEmit(it) }
        }
    ...
    actual override suspend fun requestEnableLocation() {
        // macOS doesn't have an equivalent of UIApplication.openSettingsURLString; the System
        // Settings deep link would need NSWorkspace.shared.openURL("x-apple.systempreferences:..."),
        // which is out of scope here. Consumers should surface a UI prompt instead.
    }
...
/**
 * Default [BaseLocationStateRepoBuilder] for macOS.
 */
