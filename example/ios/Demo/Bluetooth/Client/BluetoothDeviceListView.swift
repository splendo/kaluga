...
struct BluetoothDeviceListView : View {
    private let lifecycleViewModel: LifecycleViewModel<BluetoothDeviceListViewModel>
    ...
    init() {
        ...
        let viewModel = BluetoothDeviceListViewModel(alertPresenterBuilder: containerView.alertBuilder, navigator: router.navigator)
    ...
    private func deviceView(device: BluetoothDeviceListViewModel.DeviceViewModel) -> some View {
        DeviceViewModel(viewModel: device)
            .navigation(
                state: deviceRoute,
...
private struct DeviceViewModel : View {
    
    @ObservedObject var name: StringObservable
    @ObservedObject var rssi: StringObservable
    @ObservedObject var isTxPowerVisible: BoolObservable
    @ObservedObject var txPower: StringObservable
    @ObservedObject var isConnectButtonVisible: BoolObservable
    @ObservedObject var connectButton: ButtonObservable
    
    let viewModel: BluetoothDeviceListViewModel.DeviceViewModel
    
    init(viewModel: BluetoothDeviceListViewModel.DeviceViewModel) {
    ...
    var body: some View {
        HStack {
            VStack {
                Text(name.value)
                HStack {
                    Text(viewModel.identifierString)
                    Spacer()
                    Text(rssi.value)
...
extension BluetoothDeviceListView {
    ...
    class Router {
        ...
        lazy var navigator = BluetoothDeviceListNavigatorKt.BluetoothDeviceListNavigator { uuid in
            self.deviceRoutingState?.show(.details(uuid: uuid))
        }
    }
}

struct BluetoothDeviceListView_Previews: PreviewProvider {
