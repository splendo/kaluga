...
val PhysicalQuantity.MagneticDipoleMoment.converters get() = listOf<QuantityConverter<PhysicalQuantity.MagneticDipoleMoment, *>>(
    QuantityConverterWithOperator(
        "Area from Electric Current",
        QuantityConverter.WithOperator.Type.Division,
        PhysicalQuantity.ElectricCurrent,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is MagneticDipoleMoment && rightUnit is ElectricCurrent -> {
    ...
    QuantityConverterWithOperator(
        ...
        when {
            leftUnit is MagneticDipoleMoment && rightUnit is Area -> {
                DefaultScientificValue(leftValue, leftUnit) / DefaultScientificValue(rightValue, rightUnit)
            }
