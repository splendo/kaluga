...
val PhysicalQuantity.Permeability.converters get() = listOf<QuantityConverter<PhysicalQuantity.Permeability, *>>(
    QuantityConverterWithOperator(
        "Electric Inductance from Length",
        QuantityConverter.WithOperator.Type.Multiplication,
        PhysicalQuantity.Length,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is Permeability && rightUnit is Length -> {
    ...
    QuantityConverterWithOperator(
        "Magnetic Induction from Magnetic Field Strength",
        QuantityConverter.WithOperator.Type.Multiplication,
        PhysicalQuantity.MagneticFieldStrength,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is Permeability && rightUnit is MagneticFieldStrength -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }
