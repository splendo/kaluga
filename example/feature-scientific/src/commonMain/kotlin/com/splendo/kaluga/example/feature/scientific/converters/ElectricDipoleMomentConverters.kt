...
val PhysicalQuantity.ElectricDipoleMoment.converters get() = listOf<QuantityConverter<PhysicalQuantity.ElectricDipoleMoment, *>>(
    QuantityConverterWithOperator(
        ...
        when {
            leftUnit is ElectricDipoleMoment && rightUnit is Length -> {
    ...
    QuantityConverterWithOperator(
        "Length from Electric Charge",
        QuantityConverter.WithOperator.Type.Division,
        PhysicalQuantity.ElectricCharge,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is ElectricDipoleMoment && rightUnit is ElectricCharge -> {
                DefaultScientificValue(leftValue, leftUnit) / DefaultScientificValue(rightValue, rightUnit)
            }
