...
val PhysicalQuantity.Permittivity.converters get() = listOf<QuantityConverter<PhysicalQuantity.Permittivity, *>>(
    QuantityConverterWithOperator(
        "Electric Capacitance from Length",
        QuantityConverter.WithOperator.Type.Multiplication,
        PhysicalQuantity.Length,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is Permittivity && rightUnit is Length -> {
    ...
    QuantityConverterWithOperator(
        "Surface Charge Density from Electric Field Strength",
        QuantityConverter.WithOperator.Type.Multiplication,
        PhysicalQuantity.ElectricFieldStrength,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is ImperialPermittivity && rightUnit is ImperialElectricFieldStrength -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }

            leftUnit is Permittivity && rightUnit is ElectricFieldStrength -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }
