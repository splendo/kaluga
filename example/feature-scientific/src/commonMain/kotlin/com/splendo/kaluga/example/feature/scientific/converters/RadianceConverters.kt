...
val PhysicalQuantity.Radiance.converters get() = listOf<QuantityConverter<PhysicalQuantity.Radiance, *>>(
    QuantityConverterWithOperator(
        "Irradiance from Solid Angle",
        QuantityConverter.WithOperator.Type.Multiplication,
        PhysicalQuantity.SolidAngle,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is MetricRadiance && rightUnit is SolidAngle -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }

            leftUnit is ImperialRadiance && rightUnit is SolidAngle -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }
    ...
    QuantityConverterWithOperator(
        "Radiant Intensity from Area",
        QuantityConverter.WithOperator.Type.Multiplication,
        PhysicalQuantity.Area,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is MetricRadiance && rightUnit is Area -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }

            leftUnit is ImperialRadiance && rightUnit is Area -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }

            leftUnit is Radiance && rightUnit is Area -> {
                DefaultScientificValue(leftValue, leftUnit) * DefaultScientificValue(rightValue, rightUnit)
            }
