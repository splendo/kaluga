...
val PhysicalQuantity.ThermalInsulance.converters get() = listOf<QuantityConverter<PhysicalQuantity.ThermalInsulance, *>>(
    QuantityConverterWithOperator(
        "Area from Thermal Resistance",
        QuantityConverter.WithOperator.Type.Division,
        PhysicalQuantity.ThermalResistance,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is ThermalInsulance && rightUnit is ThermalResistance -> {
    ...
    QuantityConverterWithOperator(
        "Thermal Resistance from Area",
        QuantityConverter.WithOperator.Type.Division,
        PhysicalQuantity.Area,
    ) { (leftValue, leftUnit), (rightValue, rightUnit) ->
        when {
            leftUnit is ThermalInsulance && rightUnit is Area -> {
                DefaultScientificValue(leftValue, leftUnit) / DefaultScientificValue(rightValue, rightUnit)
            }
