...
fun ContactsLayout() {
    MdcTheme {
        ...
        navHostController.SetupNavHost(
            rootView = {
                ContactsListLayout(navHostController = navHostController)
            }
        ) {
            composable<ContactDetails, ContactsListNavigation.ShowContactDetails>(type = NavigationBundleSpecType.SerializedType(ContactDetails.serializer())) {
              ContactDetailsLayout(it, navHostController)
            }
        }
    }
}
