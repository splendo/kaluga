...
fun ContactsLayout() {
    MdcTheme {
        val navHostRouteController = NavHostRouteController(rememberNavController())

        navHostRouteController.SetupNavHost(
            rootView = { routeController ->
                ContactsListLayout(routeController = routeController)
            }
        ) { routeController ->
            composable<ContactDetails, ContactsListNavigation.ShowContactDetails>(type = NavigationBundleSpecType.SerializedType(ContactDetails.serializer())) { details ->
              ContactDetailsLayout(details, routeController)
            }
        }
    }
}
