...
actual class DefaultMediaManager(mediaSurfaceProvider: MediaSurfaceProvider?, private val settings: Settings, coroutineContext: CoroutineContext) :
    ...
    init {
        // macOS has no AVAudioSession — audio output routing is handled implicitly by AppKit and
        // is not controlled by category/active calls like iOS.
        launch {
    ...
    private fun createObservers() = listOf(
        ...
        observeNotification(NSApplicationDidResignActiveNotification) {
            surface?.bind?.invoke(null)
        },
        observeNotification(NSApplicationDidBecomeActiveNotification) {
            surface?.bind?.invoke(avPlayer)
        },
        // macOS has no AVAudioSession route-change notification; the corresponding
        // playAfterDeviceUnavailable handling is therefore not wired up here.
    )
    ...
    actual override fun play(rate: Float) {
        ...
        avPlayer.rate = rate
        // No AVAudioSession activation needed on macOS — output is system-managed.
    }
