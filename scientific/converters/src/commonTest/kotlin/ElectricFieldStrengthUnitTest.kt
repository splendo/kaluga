...
class ElectricFieldStrengthUnitTest {
    ...
    @Test
    fun forceFromImperialElectricFieldStrengthPreservesSystem() {
        assertEquals(PoundForce, (2(Volt per Foot) * 2(Coulomb)).unit)
        assertEquals(PoundForce, (2(Coulomb) * 2(Volt per Foot)).unit)
        assertEquals((Volt per Foot), (2(PoundForce) / 2(Coulomb)).unit)
    }
}
