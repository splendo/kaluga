...
The `KalugaViewModelLifecycleObserver` will automatically update the context of all public `LifecycleSubscribable` properties of the viewModel.
Implement this interface if a viewModel has properties that should be lifecycle or context aware.
The `UIContextObserver` is a default implementation of `LifecycleSubscribable` that provides updates to the context as a `Flow`.
