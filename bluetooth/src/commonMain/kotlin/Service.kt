 ...
 */
class Service(service: ServiceWrapper, emitNewAction: (DeviceConnectionManager.Event.AddAction) -> Unit, logger: SensitiveAwareLogger) {
    ...
    val characteristics = service.characteristics.map {
        Characteristic(
            ...
            logger = logger.withTag("${logger.tag} Service ${uuid.uuidString}"),
        )
    }
}
