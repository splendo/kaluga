...
- Use `@ByteStuffed` (CSafe-style, escapes `0xF0`-`0xF3` via `0xF3`) or `@ByteStuffedXor` (PPP/HDLC-style over an explicit byte set) to byte-stuff so a delimiter never appears literally in the content (only for `LEAST_SIGNIFICANT_FIRST` byte order). On the root structure it stuffs the body and `@Checksum` while leaving a `@Prefix`/`@Postfix` frame untouched (the CSafe frame `[start flag][stuffed body+checksum][stop flag]`). On a String or Collection property it is itself a `0x00`-terminated marking (no `@NullTerminated` needed), stuffing the content so the `0x00` terminator stays unambiguous — use `@ByteStuffedXor(escapeByte = …, escapedBytes = [0x00])`, since the scheme must escape the terminator (CSafe cannot). Add `@Terminal(byte)` to use a terminator other than `0x00`: alongside a stuffing annotation the scheme must escape that byte (and the content may contain it), while without stuffing it is a plain byte terminator (the encoded content must not contain that byte). A stuffed frame nested inside another structure is not yet supported.
