...
- Use one of `@ByteStuffed` (CSafe-style, escapes `0xF0`-`0xF3` via `0xF3`), `@ByteStuffedXor` (PPP/HDLC-style over an explicit byte set), `@ByteStuffedSlip` (SLIP/RFC 1055, delimiter `0xC0`) or `@ByteStuffedCobs` (COBS, eliminates `0x00` with ~1 byte per 254 overhead) to byte-stuff so a delimiter never appears literally in the content (only for `LEAST_SIGNIFICANT_FIRST` byte order). On the root structure it stuffs the body and `@Checksum` while leaving a `@Prefix`/`@Postfix` frame untouched (the CSafe frame `[start flag][stuffed body+checksum][stop flag]`). On a String or Collection property it is itself a `0x00`-terminated marking (no `@NullTerminated` needed), stuffing the content so the terminator stays unambiguous — the scheme must escape the terminator, so use `@ByteStuffedXor(escapeByte = …, escapedBytes = [0x00])` or `@ByteStuffedCobs` for a `0x00` terminator (CSafe cannot escape `0x00`); `@ByteStuffedSlip` defaults its terminator to its `0xC0` delimiter. Add `@Terminal(byte)` to use a terminator other than `0x00`: alongside a stuffing annotation the scheme must escape that byte (and the content may contain it), while without stuffing it is a plain byte terminator (the encoded content must not contain that byte). A stuffed frame nested inside another structure is not yet supported.
