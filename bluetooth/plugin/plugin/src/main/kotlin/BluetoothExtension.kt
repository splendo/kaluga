...
open class BluetoothExtension(private val kspExtension: KspExtension, objects: ObjectFactory) {
    ...
    /**
     * Source directories whose types are needed for cross-reference resolution but whose code is
     * already generated by another module. The KSP processor will include these as source roots
     * (so types like `BatteryService` can be referenced in `HeartRateDevice`) but will skip
     * generating `Local*` / `Remote*` / `*ReadResponse` symbols for any class whose source file
     * lives under one of these paths, preventing duplicate symbols at link time.
     */
    val externalAnnotationSourceDirectories = objects.setProperty<String>()

    /**
     * Adds a source directory as an external spec — included for type resolution only.
     * @see externalAnnotationSourceDirectories
     */
    fun externalAnnotationSource(path: String) {
        annotationSourceDirectories.add(path)
        externalAnnotationSourceDirectories.add(path)
    }
    ...
    fun afterEvaluate(resolveFile: (String) -> java.io.File) {
        ...
        val externalPaths = externalAnnotationSourceDirectories.get()
        if (externalPaths.isNotEmpty()) {
            kspExtension.arg("externalSources", externalPaths.joinToString(":") { resolveFile(it).absolutePath })
        }
    }
}
